...
const userLogued = async(req, res, next)=>{
    try {
        ...
        res.cookie('accesToken', accesToken, {maxAge: 60 * 60 * 2000, signed:true, httpOnly: true, secure: true, sameSite: 'none', domain: '.clari-ecommerce-front-9n3ry9xex-alejo-martinez.vercel.app'}).send({status:'succes', message:`Bienvenidx ${user.name} !`, payload:user});
    } catch (error) {
        next(error);
    }
}
