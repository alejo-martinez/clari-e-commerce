...
const userLogued = async(req, res, next)=>{
    try {
        ...
        res.cookie('accesToken', accesToken, {maxAge: 60 * 60 * 2000, signed:true, httpOnly: true, secure: true, sameSite: 'none', domain: '.vercel.app'}).send({status:'succes', message:`Bienvenidx ${user.name} !`, payload:user});
    } catch (error) {
        next(error);
    }
}
